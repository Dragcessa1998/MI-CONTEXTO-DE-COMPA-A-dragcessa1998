  ...
  return (
    <section className="mx-auto mt-12 max-w-md rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      ...
      <form onSubmit={submit} className="mt-6 space-y-4" noValidate>
        ...
        {mode === "register" && <>{field("name", "Nombre")}{field("phone", "Teléfono", "tel")}{field("address", "Dirección")}</>}
        {message && <p role="alert" className="text-sm text-rose-700">{message}</p>}
        <button type="submit" disabled={busy} className="w-full rounded-lg bg-brand-600 px-4 py-2.5 font-semibold text-white hover:bg-brand-700 disabled:cursor-wait disabled:opacity-60">
