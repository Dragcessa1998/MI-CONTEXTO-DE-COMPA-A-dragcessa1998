...
export default function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mode === "register" && form.password.length < 8) nextErrors.password = "Usa al menos 8 caracteres.";
    ...
    try {
      ...
      } else setMessage("No se pudo completar la operación.");
    } finally {
      setBusy(false);
    }
  }

  const input = (name: keyof RegistrationInput, label: string, type = "text") => (
    <label className="block">
      ...
      <input
        ...
        className="w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-brand-600 focus:outline-none focus:ring-2 focus:ring-brand-100"
      />
    ...
    <section className="mx-auto mt-12 max-w-md rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
      ...
      <p className="mt-5 text-center text-sm text-slate-500">
        <Link className="font-semibold text-brand-700 hover:underline" href={mode === "login" ? "/register" : "/login"}>
          {mode === "login" ? "Crear una cuenta" : "Volver al inicio de sesión"}
        </Link>
      </p>
    </section>
  );
