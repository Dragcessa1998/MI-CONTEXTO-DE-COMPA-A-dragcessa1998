...
- **Pipeline de desempeño de negocio:** paquete instalable `nexova-pipelines` con Prefect 3 y Pandas; fuente append-only `telemetry_events`; agregado semanal por `office`, `programme_id` y semana ISO; destino separado `reporting.weekly_office_program_performance`. La idempotencia combina `eventId`, reconstrucción completa de ventana y publicación transaccional por la clave natural. `services/reporting/` expone estado, disparo manual y consulta de KPI autenticados; el modo local reproduce el contrato con un export sintético y espejos ignorados en `data/eval`.
