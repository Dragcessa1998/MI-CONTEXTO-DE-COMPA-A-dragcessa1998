...
def test_agent_endpoint_is_thin_and_returns_run_id(
    ...
    response = client.post(
        "/agent/query",
        json={"question": "¿Qué SLA aplica al ticket 18?"},
    )
    assert response.status_code == 200
    assert response.json() == {
        "run_id": "run-123",
        "conversation_id": "support-user-1",
        "answer": "Respuesta trazable para: ¿Qué SLA aplica al ticket 18?",
...
def test_agent_endpoint_hides_internal_errors(
    ...
    response = client.post(
        "/agent/query", json={"question": "¿Cuál es el procedimiento SLA del ticket 18?"}
    )
